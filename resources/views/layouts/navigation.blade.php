                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @auth
                        @if (in_array(Auth::user()->role, ['admin', 'kasir']))
                            ...
                            <div class="hidden sm:flex sm:items-center" x-data="{ openCoa: false }" @click.outside="openCoa = false">
                                <div class="relative">
                                    ...
                                    <div x-show="openCoa" x-transition
                                        ...
                                        @if (Auth::user()->role === 'admin')
                                            <div class="px-4 pt-3 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider border-t border-gray-100 dark:border-gray-600 mt-1">
                                                Master Data
                                            </div>
                            ...
                            @if (Auth::user()->role === 'admin')
                                <x-nav-link :href="route('admin.commissions.index')" :active="request()->routeIs('admin.commissions.*')">
                                    ...
                                    @if ($unpaidCommissions > 0)
                                        <span class="ms-1 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold text-white bg-red-500 rounded-full">
                                            {{ $unpaidCommissions }}
                                        </span>
                                    @endif
                                </x-nav-link>
                                ...
                                <div class="hidden sm:flex sm:items-center" x-data="{ openMaster: false }" @click.outside="openMaster = false">
                                    <div class="relative">
                                        ...
                                        <div x-show="openMaster" x-transition
                                            ...
                                            <a href="{{ route('admin.services.index') }}" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 {{ request()->routeIs('admin.services.*') ? 'bg-indigo-100 text-indigo-600 font-semibold' : '' }}">
                                                Layanan
                                            </a>
                                            <a href="{{ route('admin.therapists.index') }}" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 {{ request()->routeIs('admin.therapists.*') ? 'bg-indigo-100 text-indigo-600 font-semibold' : '' }}">
                                                Terapis
                                            </a>
                                            <a href="{{ route('admin.customers.index') }}" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 {{ request()->routeIs('admin.customers.*') ? 'bg-indigo-100 text-indigo-600 font-semibold' : '' }}">
                                                Pelanggan
                                            </a>
                                            <a href="{{ route('admin.memberships.index') }}" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 {{ request()->routeIs('admin.memberships.*') ? 'bg-indigo-100 text-indigo-600 font-semibold' : '' }}">
                                                Membership
                                            </a>
                                            <a href="{{ route('admin.promos.index') }}" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 {{ request()->routeIs('admin.promos.*') ? 'bg-indigo-100 text-indigo-600 font-semibold' : '' }}">
                                                Promo
                                            </a>
                                            <a href="{{ route('admin.programs.index') }}" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 {{ request()->routeIs('admin.programs.*') ? 'bg-indigo-100 text-indigo-600 font-semibold' : '' }}">
                                                Program
                                            </a>
                                            <a href="{{ route('admin.barang.index') }}" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 {{ request()->routeIs('admin.barang.*') ? 'bg-indigo-100 text-indigo-600 font-semibold' : '' }}">
                                                Barang
                                            </a>

                                            @if (isset($customer))
                                                <a href="{{ route('admin.customers.membership.index', $customer->id) }}" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600">
                                                    Customer Membership
                                                </a>
                                            @endif
                                        </div>
