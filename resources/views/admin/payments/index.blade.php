            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700">
                    <div class="text-2xl font-bold text-gray-900 dark:text-white">Rp
                        {{ number_format($payments->sum('amount'), 0, ',', '.') }}</div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700">
                    <div class="text-2xl font-bold text-gray-900 dark:text-white">Rp
                        {{ number_format($payments->where('paid_at', '>=', now()->startOfMonth())->sum('amount'), 0, ',', '.') }}
                    </div>
                                ...
                                @foreach ($payments as $i => $payment)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        ...
                                        <td class="px-5 py-3.5 font-medium text-gray-800 dark:text-gray-200">
                                            {{ $payment->booking->customer->name ?? '—' }}</td>
                                        <td class="px-5 py-3.5 text-gray-600 dark:text-gray-400">
                                            {{ $payment->booking->service->name ?? '—' }}</td>
                                        <td class="px-5 py-3.5">
                                            @php
                                                $mc = match ($payment->method) {
                                            ...
                                            @endphp
                                            <span
                                                class="px-2.5 py-1 rounded-full text-xs font-semibold {{ $mc }}">{{ strtoupper($payment->method) }}</span>
                                        </td>
                                        <td class="px-5 py-3.5 font-semibold text-amber-600 dark:text-amber-400">Rp
