        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    Edit Membership — {{ $customer->user->name }}
                </h2>
            </div>
                ...
                <form method="POST"
                    ...
                    <div class="mb-5">
                        ...
                        <input type="date" name="start_date"
                            value="{{ old('start_date', $customerMembership->start_date->toDateString()) }}"
                            class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg text-sm focus:ring-indigo-500 focus:border-indigo-500">
                        <p class="text-xs text-gray-400 mt-1.5">Tanggal berakhir dihitung ulang otomatis dari durasi
                            membership.</p>
                        @error('start_date')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-6">
                        <label class="flex items-center gap-2.5 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1"
                                {{ $customerMembership->is_active ? 'checked' : '' }}
                                class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                            <span class="text-sm text-gray-700 dark:text-gray-300">Membership aktif</span>
                        </label>
                    </div>
